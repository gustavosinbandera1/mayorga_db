...
void CustomModel::setRelation(int foreignColumn, QString foreignTable,
  ...
  relationalModel->setTable(_table);
  relationalModel->setRelation(
      foreignColumn, QSqlRelation(foreignTable, foreignKey, columnToRender));
  // relationalModel->setRelation(0,
  // QSqlRelation("country","country_id","country_name"));
  relationalModel->select();
}
//-------------------
